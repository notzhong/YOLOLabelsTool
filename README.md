...
- **数据导出**：导出YOLO格式数据集（data.yaml + labels文件夹），支持自定义yaml文件名
...
│   │   ├── image_manager.py # 图片管理器（缓存、缩略图、批量缩放）
│   │   └── model_manager.py # YOLO模型管理器
│   ├── ui/                 # 用户界面
│   │   ├── main_window.py  # 主窗口（标注画布、菜单、快捷键）
...
### Q: 日志文件在哪里？异常信息会保存吗？
A: 日志文件位于 `logs/` 目录下，按日期命名（如 `logs/2026-04-26.log`）。程序异常会自动记录到日志文件中，包含完整的异常追踪信息。
...
### v1.4.0 (2026-04-26)
- **新增实时验证/检测功能**：支持三种检测模式（窗口捕获、区域捕获、图片文件）
- **新增 `ValidationDialog`**：基于 Win32 API + dxcam 实现高性能实时目标检测
- **窗口高亮**：选择窗口时自动高亮红色边框
- **DPI 感知**：支持多显示器不同 DPI 缩放下准确捕获
- **新增 `icon.ico`**：应用程序图标文件
- **修正项目结构文档**：移除不存在的 `tests/`、`data/` 目录，增加缺失文件说明
- **更新依赖说明**：补充 dxcam 可选依赖说明
